...
/**
 * The **hci_ecc_manager_reg_top** module is the auto-generated register file
 * top wrapper used by **hci_ecc_manager** to expose its error counters on a
 * `reg_iface` device port. It is produced by the lowRISC `reggen` tool from
 * the `hci_ecc_manager.hjson` description and contains the address decoder,
 * write-enable logic and the registered state for the four error counters
 * (see :ref:`hci_ecc_manager`).
 *
 * .. tabularcolumns:: |l|l|J|
 * .. _hci_ecc_manager_reg_top_params:
 * .. table:: **hci_ecc_manager_reg_top** design-time parameters.
 *
 *   +---------------+-------------+----------------------------------------------------------+
 *   | **Name**      | **Default** | **Description**                                          |
 *   +---------------+-------------+----------------------------------------------------------+
 *   | *reg_req_t*   | `logic`     | Type of the inbound `reg_iface` request struct.          |
 *   +---------------+-------------+----------------------------------------------------------+
 *   | *reg_rsp_t*   | `logic`     | Type of the outbound `reg_iface` response struct.        |
 *   +---------------+-------------+----------------------------------------------------------+
 *   | *AW*          | 4           | Address width of the register block (in bits).           |
 *   +---------------+-------------+----------------------------------------------------------+
 *
 * The module is not meant to be instantiated directly by end users; it is
 * exclusively used as a sub-block of **hci_ecc_manager** and is regenerated
 * from the hjson description by the `reggen` flow when the register layout
 * changes.
 */
