...
/**
 * The **hci_core_sink_v2** module acts as a job-queued wrapper around the
 * **hci_core_sink** module. It extends the functionality with a queue of
 * streamer jobs, while preserving its original behavior; please refer to
 * **hci_core_sink** for detailed functional information on the underlying
 * streamer and on all parameters not listed below.
 *
 * Whereas **hci_core_sink** accepts one job at a time through a
 ...
 * .. table:: **hci_core_sink_v2** additional design-time parameters.
 *
 *   +------------------------+-------------+------------------------------------------------------------------------------------------------------------+
...
(
  ...
  begin : wrap_fsm_comb
    ...
    case(wrap_cs)
      WRAP_IDLE : begin
        if(job_pop.valid & sink_flags.ready_start) begin
          wrap_ns   = WRAP_RUNNING;
          req_start = 1'b1;
        end
      end
      WRAP_RUNNING : begin
        if(sink_flags.done) begin
          wrap_ns       = WRAP_IDLE;
          job_pop_ready = 1'b1;
        end
      end
  ...
  assign flags_o.done               = sink_flags.done;
  assign flags_o.no_valid_transfers = sink_flags.no_valid_transfers;
  assign flags_o.addressgen_flags   = sink_flags.addressgen_flags;
