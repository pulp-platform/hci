 ...
 * i.e., greater than one bank word) that gets routed *without arbitration* to
 * `DWH/BANK_WORD_WIDTH` adjacent `out` targets from a set of `NB_OUT_CHAN` `out`
 * channels (typically, one per memory bank).
 * Routing is performed by splitting the address of the `DWH`-bit wide word in an
 * *index* and an *offset* part; the lowest `$clog2(BANK_WORD_WIDTH/BANK_ELEM_WIDTH)`
 * address bits are the element offset within a bank word and are ignored.
 ...
 * However, if `index > NB_OUT_CHAN-DWH/BANK_WORD_WIDTH`, then the set of selected
 * targets "wraps around": the first channels are activated, propagating the offset
 * incremented by one bank word.
 ...
 *   +------------------------+-------------+-------------------------------------------------------------------+
 *   | *NB_OUT_CHAN*          | 8           | Number of output HCI channels (one per memory bank).              |
 *   +------------------------+-------------+-------------------------------------------------------------------+
 *   | *BANK_WORD_WIDTH*      | 32          | Bit-width of one output bank word.                                |
 *   +------------------------+-------------+-------------------------------------------------------------------+
 *   | *BANK_ELEM_WIDTH*      | 8           | Bit-width of one element within a bank word (`be` granularity).   |
 *   +------------------------+-------------+-------------------------------------------------------------------+
 *   | *USE_ECC*              | 0           | If set to 1, enables ECC check bits propagation.                  |
 *   +------------------------+-------------+-------------------------------------------------------------------+
 *   | *FILTER_WRITE_R_VALID* | 0           | If 1, suppress r_valid responses generated by write transactions. |
 *   +------------------------+-------------+-------------------------------------------------------------------+
 */
...
(
  ...
  //NB_IN_CHAN states, to how many bank-word-wide ports the input port is equivalent
  localparam int unsigned NB_IN_CHAN  = DWH / BANK_WORD_WIDTH;
  //Word-interleaved scheme:
  // - First bits of requested address are shared
  // - Lowest ELEM_ADDR_OFFSET bits are the element offset within a bank word -> ignored
